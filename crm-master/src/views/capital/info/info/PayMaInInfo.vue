<template>
	<div class="purchase_order">
		<div class="basic_indicators" style="margin-bottom: 30px;">
			<span>{{$route.params.pay_type}}</span> - 单号:
			<span>{{$route.params.pay_number}}</span>
		</div>
...
<script>
	import {rvRecordList} from '../../../../api/api'
	export default {
		data() {
			return {
				columns5: [{
        ...
        ticket: sessionStorage.getItem("ticket"),
				refType:this.$route.params.pay_type,
				refNumber:this.$route.params.pay_number
			}).then(res=>{
