...
<script>
	import {rvDetailList} from '../../../api/api'
	export default {
		data() {
			return {
				columns5: [{
					...
					render: (h, params) => {
						return h('p', [
							h('Button', {
								...
								on: {
									click: () => {
										this.$router.push('/c/pay-mainfoinfo/'+params.row.refType+'/'+params.row.refNumber)
									}
								}
							}, '查看'),
						]);
        ...
        ticket: sessionStorage.getItem("ticket"),
				caType:this.$route.params.pay_type,
				caId:this.$route.params.pay_id
			}).then(res=>{
